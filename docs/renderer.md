...
The diff compares cells, dimensions, horizontal origin, and the surface's absolute row. Changed rows are erased and repainted, a moved surface first restores the saved cursor before clearing its old origin, and rows that disappear are explicitly erased. The native module keeps the previous frame through ordinary ZLE redisplays, so an unchanged popup produces no payload and a selection change repaints only the affected rows; explicit line teardown still clears the complete surface.

The ANSI writer only emits save/restore cursor, cursor visibility, horizontal column moves, relative row moves, targeted erase operations, style changes, and row text. When the popup would run past the bottom, it moves to the terminal's last row and emits ordinary linefeeds, so displaced host content enters terminal scrollback; it never reverse-scrolls or tries to repaint over that content. Cursor animation stays in the native shim: it queries OSC 11 and OSC 12, blends the terminal background toward the original cursor color with a 1.8-second smooth cycle driven by a 60 Hz line-editor tick, holds at full opacity for 300 ms after each edit, never dims below 40%, hides the unreliable hardware cursor, paints the active cell directly with truecolor SGR, and restores the underlying cell and real cursor on finish.
