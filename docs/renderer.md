...
The current MVP renders a rounded suggestion box below the line, or above it when the lower region is too short. Entries come from the host Zsh completion system, and the component API accepts generic ratatui widgets, so later completion panels, status surfaces, and cursor-anchored overlays can share the same measurement, buffer, diff, and patch pipeline without another terminal writer.
