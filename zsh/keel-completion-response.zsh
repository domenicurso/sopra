_keel_completion_response() {
    ...
    current_cache_key="${_KEEL_COMPLETION_LATEST_PWD}"$'\x1f'"${current_widget}"$'\x1f'"${_KEEL_COMPLETION_BROAD_BUFFER}"$'\x1f'"${_KEEL_COMPLETION_BROAD_CURSOR}"
...
_keel_completion_apply_widget() {
    ...
    if [[ $BUFFER == "$_KEEL_COMPLETION_RESPONSE_BUFFER" &&
        ...
        if (( ! applied )); then
            zle keel-native-set-suggestions "K$_KEEL_COMPLETION_RESPONSE_PAYLOAD" "$_KEEL_COMPLETION_RESPONSE_MS" && applied=1
        fi
